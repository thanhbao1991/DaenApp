...
{
    public class BaseDataProvider<T> where T : class, IHasId
    {
        ...
        private readonly string _entityName = (Activator.CreateInstance<T>() as IHasRoute)?.ApiRoute ?? typeof(T).Name.ToLower();
       ...
       senderConnectionId == AppProviders.CurrentConnectionId)
                    ...
                    await System.Windows.Application.Current.Dispatcher.InvokeAsync(async () =>
                    {
                        ...
                        string _friendlyName = TuDien._tableFriendlyNames[entityName.ToLower()];
                        ToastHelper.Show($"Cập nhật {GetActionVerb(action)} {_friendlyName.ToLower()}.");
                    });
                });
        ...
        private async Task<T?> LoadByIdAsync(string id)
        {
            try
            {
                var response = await ApiClient.GetAsync($"/api/{_entityName.ToLower()}/{id}");
