...
namespace TraSuaApp.WpfClient.AdminViews
{
    public partial class NguyenLieuTransactionList : Window
    {
        ...
        public NguyenLieuTransactionList()
        {
            ...
            this.PreviewKeyDown += NguyenLieuTransactionList_PreviewKeyDown;

            while (AppProviders.NguyenLieuTransactions?.Items == null)
            {
            ...
            AppProviders.NguyenLieuTransactions.OnChanged += () => ApplySearch();
        ...
        private void ApplySearch()
        {
            ...
            _viewSource.View.SortDescriptions.Add(new SortDescription(nameof(NguyenLieuTransactionDto.LastModified), ListSortDirection.Descending));

            var view = _viewSource.View.Cast<NguyenLieuTransactionDto>().ToList();
        ...
        {
            var window = new NguyenLieuTransactionEdit();
            window.Owner = this;
        ...
        {
            if (NguyenLieuTransactionDataGrid.SelectedItem is not NguyenLieuTransactionDto selected)
            {
                MessageBox.Show("Vui lòng chọn dòng cần sửa.", "Thông báo", MessageBoxButton.OK, MessageBoxImage.Information);
                return;
            }

            var window = new NguyenLieuTransactionEdit(selected);
            window.Owner = this;
            window.WindowStartupLocation = WindowStartupLocation.CenterOwner;
        ...
        private async void DeleteButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                var response = await ApiClient.DeleteAsync($"/api/NguyenLieuTransaction/{selected.Id}");
                var result = await response.Content.ReadFromJsonAsync<Result<NguyenLieuTransactionDto>>();
                if (result?.IsSuccess == true)
                    AppProviders.NguyenLieuTransactions.Remove(selected.Id);
                else
        ...
        private async void NguyenLieuTransactionDataGrid_MouseDoubleClick(object sender, MouseButtonEventArgs e)
        {
            if (NguyenLieuTransactionDataGrid.SelectedItem is not NguyenLieuTransactionDto selected) return;
            var window = new NguyenLieuTransactionEdit(selected);
            if (window.ShowDialog() == true)
                await AppProviders.NguyenLieuTransactions.ReloadAsync();
        }
        ...
        private void NguyenLieuTransactionList_PreviewKeyDown(object sender, KeyEventArgs e)
        {
