...
{
    private readonly INguyenLieuBanHangService _service;
    private readonly IHubContext<SignalRHub> _hub;
    string _friendlyName = TuDien._tableFriendlyNames["NguyenLieuBanHang"];

    public NguyenLieuBanHangController(INguyenLieuBanHangService service, IHubContext<SignalRHub> hub)
    {
        _service = service;
        _hub = hub;
    }
    ...
    private async Task NotifyClients(string action, Guid id)
    {
        ...
        {
            await _hub.Clients.All.SendAsync("EntityChanged", "NguyenLieuBanHang", action, id.ToString(), ConnectionId ?? "");
        }
    }
    ...
    public async Task<ActionResult<Result<NguyenLieuBanHangDto>>> GetById(Guid id)
    {
        ...
        return Result<NguyenLieuBanHangDto>.Success(dto);
    }
    ...
    [HttpPut("{id}/restore")]
    public async Task<ActionResult<Result<NguyenLieuBanHangDto>>> Restore(Guid id)
    {
    ...
    [HttpGet("sync")]
    public async Task<ActionResult<Result<List<NguyenLieuBanHangDto>>>> Sync(DateTime lastSync)
    {
